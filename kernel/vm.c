...
#include "virtio.h" // For bread/brelse
...
#define SWAP_BLOCK_SIZE 512  // Match disk sector size
struct swap {
  uint block_start;    // First disk block for swap
  uint block_count;   // Number of blocks available
  uchar *bitmap;      // Use xv6's uchar instead of uint8_t
  struct spinlock lock;
};
struct swap swapdev;
/*****************************************/
// orignal swapinit function
void swapinit() {
  initlock(&swapdev.lock, "swap");
  swapdev.block_start = 1000;   // Start at disk block 1000
  swapdev.block_count = 1024;   // 1024 blocks = 512KB swap
  swapdev.bitmap = (uchar*)kalloc();
  memset(swapdev.bitmap, 0, 1024/8);
  printf("swap: initialized at block %d\n", swapdev.block_start);
}


// creating the swapput function
// Add after swapinit()
int swapin(struct proc *p, uint64 va) {
  ...
  // 2. Get swap slot
  int slot = (*pte >> 10) & 0x3FFFFF;
  
  // 3. Allocate page and buffer
...
int swapout(struct proc *p, uint64 va) {
  ...
  for (int i = 0; i < swapdev.block_count; i++) {
    if ((swapdev.bitmap[i/8] & (1 << (i%8))) == 0) {
      slot = i;
      swapdev.bitmap[i/8] |= (1 << (i%8));  // Mark slot used
      break;
    }
  }
  if (slot < 0) {
...
//   12..20 -- 9 bits of level-0 index.S
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  ...
  // --- ADD DEBUG HERE --- //
  // pte_t *leaf_pte = &pagetable[PX(0, va)];
  // if(*leaf_pte & PTE_SWAPPED) {
  //   printf("walk: swapped page detected for va %lx\n", va);
  // }
  // return leaf_pte;
  // --------------------- //
  return &pagetable[PX(0, va)];  // THIS LINE WAS MISSING
}
...
//---------------------------------------------------------------------------------------
// i will not delet this function just in case i need it later
void
...
// update: so deepthink thinks that his function is better than the original one
// so i am keeping it incase he was wrong #potential_issue
//---------------------------------------------------------------------------------------
// new uvmunmap function
